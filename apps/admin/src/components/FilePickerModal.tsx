...
export default function FilePickerModal({ show, onHide, onSelect }: FilePickerModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      let filename = deleteTarget.filename || '';
      if (deleteTarget.url && deleteTarget.url.startsWith('/uploads/')) {
        filename = deleteTarget.url.replace('/uploads/', '');
      }
            ...
            if (filteredFiles.length === idx + 1) {
              return (
                <div ref={lastFileRef} key={file.id || file.url || idx} className={`border rounded p-2 bg-white shadow-sm ${colClass}`} style={{ minWidth: 200, maxWidth: 220 }}>
                  ...
                  <div className="d-flex gap-2 mt-2">
                    <Button variant="outline-primary" size="sm" className="w-100" onClick={() => onSelect(file)}>
                      Выбрать
                    </Button>
                    <Button variant="outline-danger" size="sm" className="w-100" onClick={() => setDeleteTarget(file)}>
                      Удалить
                    </Button>
                  </div>
                </div>
              ...
              return (
                <div key={file.id || file.url || idx} className={`border rounded p-2 bg-white shadow-sm ${colClass}`} style={{ minWidth: 200, maxWidth: 220 }}>
                  ...
                  <ConfirmModal
                    show={!!deleteTarget}
                    onHide={() => { setDeleteTarget(null); setDeleteError(''); }}
                    onConfirm={handleDelete}
                    title="Удалить файл?"
                    body={deleteTarget ? `Вы уверены, что хотите удалить файл "${deleteTarget.originalName ?? deleteTarget.filename}"? Это действие нельзя отменить.` : ''}
                    confirmText="Удалить"
